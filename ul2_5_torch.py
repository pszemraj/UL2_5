...
class UL25DataCollator:
    ...
    def _get_prefix_ids(self, prefix: str, device: torch.device) -> Optional[Tensor]:
        ...
        if not prefix:
            return None
        return self._prefix_cache.get(prefix, torch.tensor([], dtype=torch.long)).to(device)
...
def _run_tests():
    ...
    print(f"  span_corruption_mask: {mask.sum().item()}/{seq_len} masked (density={density:.3f})")
    ...
    print(f"  input_ids: {batch['input_ids'].shape}, device={batch['input_ids'].device}")
