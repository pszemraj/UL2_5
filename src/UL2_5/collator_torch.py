"""Pure PyTorch UL2.5 Data Collator (no HF dependency for collation)."""
...
class UL25DataCollator:
    """
    UL2.5 Data Collator for encoder-decoder models (pure PyTorch).

    Compatible with PyTorch DataLoader. Does not require transformers
    for the collator itself (only for tokenization).

    Args:
        tokenizer: Tokenizer with extra_id sentinel tokens
    ...
    def __init__(
        ...
        self.eos_id = getattr(tokenizer, "eos_token_id", 1)
    ...
    def _get_sentinel_start(self) -> int:
        ...
        all_special_tokens = getattr(self.tokenizer, "all_special_tokens", [])
        all_special_ids = getattr(self.tokenizer, "all_special_ids", [])
        extra_ids = []
        for i, tok in enumerate(all_special_tokens):
            if "extra_id" in tok.lower():
                extra_ids.append(all_special_ids[i])
        if extra_ids:
    ...
    def _get_prefix_ids(self, prefix: str, device: torch.device) -> Tensor | None:
    ...
    def _pad_to_multiple(self, length: int, cap: int | None = None) -> int:
    ...
    def __call__(self, examples: list[dict[str, Any]]) -> dict[str, Tensor]:
        ...
        max_enc = self._pad_to_multiple(max_enc, self.max_length)
        max_dec = self._pad_to_multiple(max_dec, self.max_labels_length)
