...
if TORCH_AVAILABLE:
    ...
    def _random_segmentation(n_items: int, n_segments: int, device: torch.device) -> Tensor:
        ...
        ends = torch.cat([dividers + 1, torch.tensor([n_items], dtype=torch.long, device=device)])
    ...
    def prefix_lm_mask(seq_len: int, mode: str, device: torch.device) -> Tuple[Tensor, int]:
        ...
        if mode == "random":
            split = torch.randint(int(0.2 * seq_len), int(0.8 * seq_len) + 1, (1,), device=device).item()
        elif mode == "short":
    ...
    def infilling_mask(seq_len: int, hole_frac: float, device: torch.device) -> Tuple[Tensor, int, int]:
    ...
    def create_sentinel_ids(mask: Tensor, sentinel_start: int) -> Tensor:
        ...
        sentinel_ids = torch.where(span_starts, sentinel_start - cumsum + 1, torch.zeros_like(cumsum))
        
        continuations = mask & ~span_starts
        sentinel_ids = torch.where(continuations, torch.full_like(sentinel_ids, -1), sentinel_ids)
    ...
    def apply_sentinel_mask(
        ...
        if eos_id is not None:
            result = torch.cat([result, torch.tensor([eos_id], dtype=result.dtype, device=device)])
...
class UL25DataCollator(DataCollatorMixin if HF_AVAILABLE else object):
    ...
    def _get_sentinel_start(self) -> int:
        ...
        if not extra_ids:
            warnings.warn("No extra_id tokens found. Using 32099 as default sentinel start.")
            return 32099
    ...
    def _generate_mask(self, seq_len: int, spec: DenoiserSpec, device: torch.device) -> Tensor:
    ...
    def _process_single(self, input_ids: Tensor) -> Dict[str, Tensor]:
        ...
        weights = torch.tensor(self.config.get_weights(self._progress), dtype=torch.float32)
        ...
        # Apply masks
        encoder_ids = apply_sentinel_mask(input_ids, enc_sentinels, prefix_ids, self.eos_id)
    ...
    def _pad_length(self, length: int) -> int:
        ...
        if self.pad_to_multiple_of is None:
            return length
        return ((length + self.pad_to_multiple_of - 1) // self.pad_to_multiple_of) * self.pad_to_multiple_of
    ...
    def torch_call(self, examples: List[Dict[str, Any]]) -> Dict[str, Tensor]:
        ...
        max_enc = min(self.max_length, max(p["encoder_ids"].shape[0] for p in processed))
        max_dec = min(self.max_labels_length, max(p["decoder_ids"].shape[0] for p in processed))
        ...
        # Allocate tensors
        input_ids = torch.full((batch_size, max_enc), self.pad_id, dtype=torch.long, device=device)
        attention_mask = torch.zeros((batch_size, max_enc), dtype=torch.long, device=device)
        labels = torch.full((batch_size, max_dec), -100, dtype=torch.long, device=device)
...
def _test():
    ...
    for name in ["recommended", "curriculum", "ul2", "t5", "minimal"]:
        config = getattr(UL25Config, name if name != "curriculum" else "recommended_with_curriculum")()
    ...
    examples = [{"input_ids": torch.randint(100, 1000, (seq_len,), device=device)} for _ in range(batch_size)]
