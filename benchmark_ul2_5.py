...
def test_masking_correctness(device: torch.device):
    ...
    print("  Hole frac=0.3, expected size≈30")
    print("  ✓ Passed")
    ...
    collator = UL25DataCollator(
        tokenizer, UL25Config.recommended(), max_length=128, max_labels_length=64
    )
    ...
    assert (batch["attention_mask"].sum(dim=1) > 0).all(), (
        "Each example should have some attention"
    )
...
def benchmark_full_collator(device: torch.device):
    ...
    for config_name, config in configs.items():
        collator = UL25DataCollator(
            tokenizer, config, max_length=1024, max_labels_length=256
        )
        ...
        for batch_size in batch_sizes:
            for seq_len in seq_lens:
                ...
                print(
                    f"{config_name:<15} {batch_size:<8} {seq_len:<8} {time_ms:<12.2f} {samples_per_sec:<12.0f} {tokens_per_sec:<15,.0f}"
                )
...
def benchmark_cpu_vs_gpu():
    ...
    for device_name, metrics in results.items():
        print(
            f"{device_name:<10} {metrics['time_ms']:<15.2f} {metrics['tokens_per_sec']:<20,.0f}"
        )
...
def create_visualizations(device: torch.device):
    ...
    ax.set_title(
        f"Masking Function Benchmark ({device})",
        color="white",
        fontsize=14,
        fontweight="bold",
    )
